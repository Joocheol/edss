...
- `build_edss_duckdb.py`: 233개 이질적 패널을 소스별 독립 테이블로 하나의 제한 DuckDB에 재개 가능하게 적재한다. 업무열 수와 공통 출처열 12개를 분리 검증하고, 학교–연도 핵심 마트·파일 연도 취업 마트뿐 아니라 승인된 2010–2020 취업 코호트 마트와 무증식 결합 뷰를 함께 만든다.
