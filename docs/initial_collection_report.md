...
- 다른 주제의 학교연도 키 가운데 `0101 고등교육학교개황`에 없는 키가 316개다.
...
`python3 -m unittest discover -s tests -p 'test_*.py' -v` 결과 24개 테스트가 모두 통과했다. 기존 API·다운로드 검사에 더해 중첩 ZIP, CP949 표본 경계, 중복 헤더, 문자열 식별자 보존, 전체연도·단일연도 판별, 제공연도 범위 결합, 스키마에서 사라진 식별자 결측 검사를 포함한다.

## 다음 수집 대상과 필요한 조치

1. API 일일 한도가 초기화되면 학교 ID·학교명 교차표와 최신 상태를 사후 검증한다. EDSS 원자료를 API로 대체하지 않는다.
2. `0101`에 연결되지 않는 316개 학교연도 키의 조사범위·학교상태 차이를 확인한다.
