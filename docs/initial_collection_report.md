...
- 공공데이터포털·대학알리미 원본 11개와 EDSS 원본 ZIP 1개, 총 12개 원본(114,812,650 bytes)을 확보했다. 각 파일의 출처, 크기, SHA-256, 기준연도, 라이선스를 매니페스트에 기록했다.
- EDSS 우선순위 1의 14개 물리 데이터셋에서 연도별 파일 식별자와 `ALL` 파일 존재를 확인했다. 브라우저에서 `0101 고등교육학교개황` 전체 다운로드는 성공했지만 독립 HTTP 수집기는 계속 404를 반환했다.
...
EDSS 웹 브라우저에서 `0101 고등교육학교개황`의 전체 다운로드를 실행해 `2009~2025` ZIP을 확보했다. ZIP은 1,100,924 bytes이고 SHA-256은 `cb8e04e04d18de258343a64a6eddf60e4da9cf3c1aaa65deacd6697252e6e4d2`다. 내부에는 CP949 CSV 17개가 있으며 2009~2025가 각각 한 파일에 대응한다. 모든 파일은 동일한 66열 헤더를 사용하고 전체 데이터 행은 31,883개다.

반면 독립 HTTP 수집기의 `/es_opd_oddod01_005` 요청은 `대학교학생개황`(`domnCd=16920`) 재시험에서도 HTTP 404를 반환했다. 따라서 기존의 “공식 파일 또는 서버 저장 문제” 판단을 수정하고, 브라우저 세션·쿠키·요청 문맥을 재현하지 못한 자동 수집 방식의 문제로 분류했다. 브라우저 수동 다운로드 원본은 `data/metadata/edss_file_manifest.jsonl`, 스키마와 연도별 행 수는 `data/metadata/edss_0101_schema.json`, 자동 요청 실패는 `data/metadata/edss_download_attempts.jsonl`에 구분해 기록했다.
...
2. EDSS 브라우저 다운로드 문맥을 자동 수집기에 재현하거나, 같은 방식으로 나머지 우선순위 1 `ALL` ZIP을 수동 확보한다.
