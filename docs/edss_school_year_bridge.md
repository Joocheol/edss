...
`data/metadata/edss_school_year_bridge.csv`는 카탈로그에 등록된 전체 233개 패널 중 하나 이상에서 관측된, 비어 있지 않은 `(_panel_year, 개방ID)`마다 정확히 한 행을 갖는 결합 기준표다. 다른 패널을 `0101 고등교육학교개황`의 학교·캠퍼스 속성과 결합할 때 원시 `0101`을 직접 조인해서 생기는 행 증식을 막는 것이 목적이다.
...
| `_source_dataset_count`, `_source_catalog_codes`, `_source_row_count` | 해당 키를 가진 전체 논리 패널 수, `분야:카탈로그코드` 목록과 행 수 |

목록 열은 중복을 제거하고 사전순으로 정렬한 뒤 `|`로 연결한다. 빈 값은 목록과 개수에서 제외하지만 `_0101_source_row_count`는 원본 행 전체를 세므로, 원본 행 수와 속성 목록의 수를 혼동하지 않는다.

## 미연결과 검토 상태

미연결 780개 키도 기준표에서 삭제하지 않는다. `_0101_exists=false`로 두고 다음 상태를 보존한다.
...
- 기준표: 31,336행, 고유키 31,336개, 최대 키 반복 수 1
- `0101` 연결: 30,556개 키
- `0101` 미연결: 780개 키(최초 관측 전 63, 마지막 관측 후 626, 내부 공백 49, 전 기간 미등장 42)
- 캠퍼스 표시: 단일 29,369개, 복수 1,187개, `0101` 미관측 780개
- 입력: 233개 패널 180,119,183행, 체크섬 불일치 0개
- 결합 키 결측: 취업통계 구조 전환의 46,962행. 사실 패널에 그대로 보존
- `0101` 미연결 영향: 82,959행
- 기준표 left join: 출력 180,119,183행, 행 증식 0

새 기준표의 미연결 780개 키와 분류는 전체 감사의 `edss_full_panel_orphan_school_year_keys.csv`를 패널 간 중복 제거한 키 집합 및 분류와 정확히 일치한다.
