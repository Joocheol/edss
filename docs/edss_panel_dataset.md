...
공식 교차표 후속 감사에서도 후보는 확정되지 않았다. 2026년 8월 EDSS 제공목록은 2023–2024년 학교코드 제공 여부를 `Y`로 표시하지만, 체크섬이 확인된 실제 두 ZIP의 24개 열에는 `학교명`만 있고 `개방ID`와 다른 코드형 학교 열은 없다. 이는 취업 집계 수치의 오류가 아니라 학교 코드 제공 메타데이터의 불일치다. 공식 서면 확인 전까지 2023–2024년을 OpenID 장기 패널에 강제 연결하지 않는다.

대학알리미 재적학생수 교차검증에서는 2023·2024년 학교구분·지역·본분교·재적학생수가 각 연도에 하나의 EDSS OpenID와 정확히 일치하고 두 해의 OpenID도 같은 학교 245개를 찾았다. 역방향 중복은 0개이며 취업 학교·연도 482개에 이름으로 연결됐다. 기존 학과서명 후보와 겹치는 24개는 모두 동의했다. 검토 승인 후 별도 `employment_2023_2024_school_department_resolved.csv.gz`에만 적용해 13,920행·482개 학교연도·242개 고유 OpenID를 채웠다. 33,042행은 미해결로 남겼고 기존 값 덮어쓰기와 충돌은 0건이다. 적용값은 공식 교차표가 아닌 추론값임을 별도 근거 열로 보존한다.
