...
2026-09-02 후속 실행에서 이 46,962행을 개인형 열이 없는 학교·학과 집계 파생 파일로 분리했다. 과거 OpenID별 학과·단과대 완전 집합과 일치한 학교·연도 후보 32개 중 30개는 같은 연도 `0101` 시도·본분교 맥락도 일치했고 2개는 충돌했다. 이 후보 전용 기본 파생 파일에는 정식 `개방ID`를 대입하지 않는다. 자세한 규칙과 고위험 패널 검토는 `docs/edss_remaining_identity_gap_resolution.md`에 기록했다.

공식 교차표 후속 감사에서도 후보는 확정되지 않았다. 2026년 8월 EDSS 제공목록은 2023–2024년 학교코드 제공 여부를 `Y`로 표시하지만, 체크섬이 확인된 실제 두 ZIP의 24개 열에는 `학교명`만 있고 `개방ID`와 다른 코드형 학교 열은 없다. 이는 취업 집계 수치의 오류가 아니라 학교 코드 제공 메타데이터의 불일치다. 2023–2024년 46,962행 전체는 `schema_break_excluded`로 승인해 OpenID 장기 패널에서 제외하고 독립 참고 자료로만 보존한다.

대학알리미 재적학생수 교차검증에서는 2023·2024년 학교구분·지역·본분교·재적학생수가 각 연도에 하나의 EDSS OpenID와 정확히 일치하고 두 해의 OpenID도 같은 학교 245개를 찾았다. 역방향 중복은 0개이며 취업 학교·연도 482개에 이름으로 연결됐다. 기존 학과서명 후보와 겹치는 24개는 모두 동의했다. 별도 `employment_2023_2024_school_department_resolved.csv.gz`의 13,920행·482개 학교연도·242개 고유 OpenID는 추론 적용 기록으로 보존하고 33,042행은 공란을 유지한다. 두 집합 모두 기존 종단 패널에는 넣지 않으며, 적용값이 공식 교차표가 아니라는 근거 열도 유지한다.
...
- 취업통계 2023–2024년 후보 전용 안전 파생 집계 46,962행; 개인형 열 0개, `개방ID` 대입 0건
- 별도 OpenID 적용 안전 파생 집계 46,962행; 13,920행 적용, 33,042행 미해결, 행 손실·기존 ID 덮어쓰기·충돌 0건
- 취업통계 2023–2024년 기존 OpenID 종단 패널 적격 행 0건; 46,962행 전체를 형식 단절 범위로 제외하고 독립 보존
- high 패널 4개 중 3개는 시간 경계로 설명, `1209`의 내부 공백 1개·16행은 남인천캠퍼스 학위과정 범위 공백으로 수동 확정하여 검토 잔여 0개
- 공식 교차표 확정 후보 0개; 공식 목록의 학교코드 제공 `Y`와 실제 파일의 코드 열 부재가 충돌
- 대학알리미 2개 연도 정확일치 후보 245개·취업 학교연도 연결 482개·학과서명 동의 24개·충돌 0개; 승인 후 별도 안전 파생 패널 13,920행에 적용
