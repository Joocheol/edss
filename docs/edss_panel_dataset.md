...
다른 주제의 고유 학교연도 키를 `0101`과 비교하면 데이터셋별 연결률은 99.30–100%다. 데이터셋별 미연결 키를 합하면 316건(일반 패널 305건, 취업통계 11건)이지만, 동일 키의 데이터셋 간 중복을 제거하면 76개 학교연도 키와 69개 `개방ID`다. 영향 행은 8,106행으로 전체의 0.0594%다.
