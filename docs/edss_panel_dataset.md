...
`0101 고등교육학교개황`에는 31,883행과 30,556개의 `(연도, 개방ID)`가 있다. 이 후보키가 반복되는 키는 1,187개이고 최대 반복 수는 4이므로 학교연도 기준표도 추가 원본 차원 없이 일대일 테이블로 간주할 수 없다.

다른 주제의 고유 학교연도 키를 `0101`과 비교하면 연결률은 99.30–100%다. 전체 316개 키가 `0101`에 없으며, 이 가운데 일반 패널이 305개, 취업통계가 11개다. 이는 조사대상·공시범위·학교 상태 차이일 수 있으므로 삭제하거나 임의 매핑하지 않고 검토 대상으로 보존한다.
...
검증 상태는 `review_required`다. 파일 손상이나 연도 누락 때문이 아니라 학교연도 미연결 316개와 취업통계의 2023년 스키마 단절을 사후 검증해야 하기 때문이다.
