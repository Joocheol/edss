...
- **처리:** 후보 파일과 원본·제한 패널은 그대로 유지한다. 별도 안전 파생 패널의 13,920개 적용 행에는 방법·상태·후보 파일 경로를 기록하되, 후속 형식 단절 결정에 따라 기존 OpenID 종단 분석에서는 제외한다.
- **잔여 위험:** 33,042행(70.3590%)에는 승인 후보가 없고, 적용된 13,920행의 ID도 공식 교차표가 아닌 추론값이다. 따라서 전체 46,962행을 독립 참고 자료로만 사용한다.
